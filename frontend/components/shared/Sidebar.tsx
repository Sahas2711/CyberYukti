    ...
    items: [
      {
        ...
        match: "exact",
        icon: (active) => (
          <svg className={`h-4 w-4 ${active ? "text-accent" : "text-tx-tertiary"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        match: "priority",
        icon: (active) => (
          <svg className={`h-4 w-4 ${active ? "text-priority-p1" : "text-priority-p1/70"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    items: [
      {
        ...
        match: "evidence",
        icon: (active) => (
          <svg className={`h-4 w-4 ${active ? "text-evidence-confirmed" : "text-evidence-confirmed/70"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 hidden h-screen w-[240px] flex-col border-r border-line bg-graphite transition-colors lg:flex">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[15px] font-black tracking-wider text-tx-primary font-mono">
                CYBERYUKTI
              </span>
            </div>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <div key={item.href} className="relative">
                    ...
                    <Link
                      href={item.href}
                      className={`flex items-center justify-between rounded-md px-3 py-2 text-[13px] font-medium transition-all ${
                        active
      ...
      <div className="shrink-0 border-t border-line bg-graphite-raised/50 p-4">
        ...
        <p className="text-[11px] leading-relaxed text-tx-tertiary">
          GitLab Triple-Tuple deduplication & cross-tool SAST-DAST correlation active.
        </p>
      </div>
    </aside>
  );
