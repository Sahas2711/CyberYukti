...
export default function DashboardPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const [statsResult, casesResult, ingestionResult] = await Promise.all([
        provider.getDashboardStats(),
        provider.listCases(),
        provider.getIngestion(),
      ]);
      setStats(statsResult);
      setCases(casesResult);
      setClusters(ingestionResult.clusters);
      setIngestion(ingestionResult.summary);
    } catch (err) {
  ...
  const pipelineSummary = ingestion ?? null;
    ...
    <div className="mx-auto max-w-[1400px] space-y-6 px-5 py-6 lg:px-8">
      <section className="space-y-2">
        ...
        <p className="text-xl font-semibold tracking-tight text-tx-primary sm:text-2xl">
          {pipelineSummary
            ? `${pipelineSummary.total_raw_findings} raw findings reduced to ${pipelineSummary.total_clusters} correlated clusters → ${cases.length} triage cases.`
            : `${stats.total_findings} findings reduced to ${stats.unique_clusters} actionable vulnerability case${
                stats.unique_clusters === 1 ? "" : "s"
              }.`}
        </p>
      </section>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <IngestionPanel summary={pipelineSummary} />
